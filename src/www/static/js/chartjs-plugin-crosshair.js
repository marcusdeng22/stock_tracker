...
function TracePlugin(Chart) {
	...
	var crosshairPlugin = {
		...
		afterInit: function(chart) {
			...
			var yScaleType = chart.config.options.scales.yAxes[0].type;

			if (yScaleType !== 'linear' && yScaleType !== 'time' && yScaleType !== 'category' && yScaleType !== 'logarithmic' && yScaleType !== "financialLinear") {
				console.log(yScaleType)
				return;
			}
		...
		afterEvent: function(chart, e) {
			...
			// if (zoomEnabled && buttons === 1 && !chart.crosshair.dragStarted) {
